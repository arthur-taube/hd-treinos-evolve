...
interface MuscleGroupDialogAdvancedProps {
  open: boolean;
  onClose: () => void;
  onSelect: (groups: string[], isMultiple: boolean) => void;
}

const MuscleGroupDialogAdvanced = ({ open, onClose, onSelect }: MuscleGroupDialogAdvancedProps) => {
  ...
  useEffect(() => {
    const fetchMuscleGroups = async () => {
      ...
      setIsLoading(true);
      console.log("Fetching advanced muscle groups from database...");
      
      try {
        const { data, error } = await supabase
          .rpc('get_distinct_muscle_groups_avancado');

        if (error) {
          console.error('Error fetching advanced muscle groups:', error);
          return;
        }
        ...
        console.error('Exception while fetching advanced muscle groups:', error);
      } finally {
        setIsLoading(false);
      }
    };
...
export default MuscleGroupDialogAdvanced;
